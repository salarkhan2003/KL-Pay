...
>
 ...
 can't be undone.</p>
      ...
      </motion.divn.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
        ...
        <p className="text-white/30 text-sm mb-6">Thisg-widest text-white/30 mb-1.5 flex items-center gap-1.5">{icon}{label}</p>
    {children}
  </div>
);
...
const ConfirmDelete: React.FC<{ label: string; onCancel: () => void; onConfirm: () => Promise<void> }> = ({ label, onCancel, onConfirm }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motioems-center justify-between">
...
const Field: React.FC<{ label: string; icon: React.ReactNode; children: React.ReactNode }> = ({ label, icon, children }) => (
  <div>
    <p className="text-[10px] font-black uppercase trackin    className="fixed inset-0 z-[200] flex items-end justify-center p-4 bg-black/70 backdrop-blur-md"
    ...
    <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }}
      ...
      <div className="flex it.id, deleteItem.outletId); setOutletMenus(prev => { const copy = { ...prev }; delete copy[deleteItem.outletId]; return copy; }); setDeleteItem(null); }} />
        )}
      </AnimatePresence>
    </motion.div>
  );
...
const Modal: React.FC<{ title: string; onClose: () => void; children: React.ReactNode }> = ({ title, onClose, children }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      {deleteOutletId && (
          <ConfirmDelete label="outlet" onCancel={() => setDeleteOutletId(null)} onConfirm={async () => { await onDeleteOutlet(deleteOutletId); setDeleteOutletId(null); }} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {deleteItem && (
          <ConfirmDelete label="menu item" onCancel={() => setDeleteItem(null)} onConfirm={async () => { await onDeleteMenuItem(deleteItem item: { ...p.item, imageUrl: e.target.value } } : null)} />
            </Field>
  ...
  ,
            ...
            <Field label="Image URL (optional)" icon={<Link className="w-4 h-4" />}>
              <input className="input-field" placeholder="https://..." value={itemModal.item.imageUrl || ''}
                onChange={e => setItemModal(p => p ? { ...p,alue } } : null)} />
              </Field>
            </div>
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {ITEM_CATS.map(cat => (
                  <button key={cat} onClick={() => setItemModal(p => p ? { ...p, item: { ...p.item, category: cat } } : null)}
                    className={cn("px-3 py-1.5 rounded-full text-xs font-black border transition-all"" inputMode="numeric" placeholder="0" value={itemModal.item.price || ''}
              ...
              <Field label="Prep Time" icon={<Clock className="w-4 h-4" />}>
                <input className="input-field" placeholder="10m" value={itemModal.item.prepTime || ''}
                  onChange={e => setItemModal(p => p ? { ...p, item: { ...p.item, prepTime: e.target.v <Field label="Item Name" icon={<Tag className="w-4 h-4" />}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Price (₹)" icon={<CreditCard className="w-4 h-4" />}>
                <input className="input-field" type="numberleSaveOutlet} className="w-full h-12" disabled={savingOutlet || !outletModal.name}>
      ...
      <AnimatePresence>
        {itemModal && (
          <Modal title={itemModal.item.id ? 'Edit Item' : 'Add Menu Item'} onClose={() => setItemModal(null)}>
           nput-field" placeholder="https://..." value={outletModal.imageUrl || ''}
            ...
            <ClayButton onClick={hand<option key={c} value={c}>{c}</option>)}
                </select>
              </div>
            </div>
            ...
            <Field label="Image URL (optional)" icon={<Link className="w-4 h-4" />}>
              <input className="i..p, blockName: e.target.value }))}>
              ...
              <div>
                ...
                <select className="input-field" value={outletModal.category || 'Meals'}
                  ...
                  {CATEGORIES.map(c => ld" placeholder="Short description" value={outletModal.description || ''}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="input-field" value={outletModal.blockName || 'CSE'}
                  onChange={e => setOutletModal(p => ({ .title={outletModal.id ? 'Edit Outlet' : 'Add Outlet'} onClose={() => setOutletModal(null)}>
            ...
            <Field label="Description" icon={<Tag className="w-4 h-4" />}>
              <input className="input-fieclassName="p-5">
      ...
      <AnimatePresence>
        {outletModal && (
          <Modal                        className="w-7 h-7 rounded-lg bg-red-500/10 flex items-center justify-center text-red-400">
      ...
      {/* System tools */}
      <GlassCard             <div className="flex gap-1.5 flex-shrink-0">
       ...
       -0">
              ...
              >
                  {(outletMenus[outlet.id] || []).length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {(outletMenus[outlet.id] || []).map(item => (
                        <div key={item.id} className="flex items-center gap-3 bg-white/3 rounded-xl p-2.5">
                          <div className="w-8 h-8 rounded-lg overflow-hidden bg-white/5 flex-shrink mb-3">
                  ...
                  </divvronDown className="w-3.5 h-3.5" />}
                </button>
              </div>
            </div>
            ...
            <AnimatePresence>
              {expandedOutlet === outlet.id && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  ...
                  <div className="flex items-center justify-between pt-3assName="w-8 h-8 rounded-xl bg-red-500/10 flex items-center justify-center text-red-400">
                ...
                <button onClick={() => setExpandedOutlet(expandedOutlet === outlet.id ? null : outlet.id)}
                  ...
                  {expandedOutlet === outlet.id ? <ChevronUp className="w-3.5 h-3.5" /> : <Checategory} · {outlet.upiId || 'No UPI'}</p>
              </div>
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button onClick={() => setDeleteOutletId(outlet.id)}
                  clsName="flex items-center gap-3 p-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-white/30">{outlet.blockName} · {outlet.ton onClick={() => setOutletModal({ ...EMPTY_OUTLET })}
        ...
        {outlets.map(outlet => (
          <div key={outlet.id} className="glass-frosted rounded-[24px] border border-white/10 overflow-hidden">
            {/* Outlet row */}
            <div clas"h-full bg-klu-red rounded-full" />
                </div>
              </div>
            );
          })}
            ...
            return (
              <div key={block} className="space-y-1">
                ...
                <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} className=ame="text-[10px] text-white/30 font-black uppercase">Platform Fees</p>
      ...
      <GlassCard>
        ...
        <div className="space-y-3">
          {BLOCKS.slice(0, 5).map(block => {
            const count = allOrders.filter(o => o.block === bloName="text-display text-3xl font-black">Admin Panel</h2>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
      <h2 class`outlets/${expandedOutlet}/menu`),
  ...
  const handleSaveItem = async () => cord<string, MenuItem[]>>({});
  ...
  // Load menu for expanded outlet
  useEffect(() => {
    if (!expandedOutlet) return;
    if (outletMenus[expandedOutlet]) return; // already loaded
    const unsub = onSnapshot(
      collection(db, totalAmount, 0);
  ...
  // Expanded outlet (to manage its menu inline)
  const [expandedOutlet, setExpandedOutlet] = useState<string | null>(null);
  const [outletMenus, setOutletMenus] = useState<ReET: Partial<Outlet> = { name: '', description: '', blockName: 'CSE', category: 'Meals', upiId: '', imageUrl: '', isOpen: true, timings: '8am – 9pm' };
...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const totalRevenue = allOrders.reduce((acc, o) => acc + o.'];
const ITEM_CATS  = ['Main', 'Snack', 'Breakfast', 'Beverage', 'Juice', 'Dessert'];
const EMPTY_OUTLata: () => void;
  isSeeding: boolean;
...
const CATEGORIES = ['Meals', 'Cafe', 'Bakery', 'Juice', 'Snacks
  ...
  X, Store, Tag, MapPin, Link, CreditCard, Clock, ChevronDown, ChevronUp
} from 'lucide-react';
...
interface AdminViewProps {
  allOrders: Order[];
  outlets: Outlet[];
  onSeedDimport { motion, AnimatePresence } from 'motion/react';
import {